      ...
      <div className="card animate-fadeIn border-l-4 border-blue-500">
        ...
        {isResult && (
          <div className="text-sm text-text-secondary mb-4">
            Votação encerrada • Resultado até {formatDateTimeBR(votacao.data_resultado_ate)}
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between mt-2">
              <p className="text-xs text-text-secondary">
                Até {formatDateTimeBR(votacao.data_limite)}
              </p>
              {alterandoVoto && (
          ...
          <div className="space-y-3">
            ...
            {podeAlterar && (
              <p className="text-xs text-text-secondary -mt-1">
                Pode alterar até {formatDateTimeBR(votacao.data_limite)}
              </p>
            )}
            {votacao.escolhas?.map((escolha) => {
              const isMyVote = escolha.id === votacao.escolha_usuario;
        ...
        {eventosAtivos.length > 1 && (
          <div className="mb-8 flex flex-wrap gap-2">
            {eventosAtivos.map(evt => (
              ...
              >
                {evt.tipo === 'RELAMPAGO' && '⚡ '}
                {evt.nome || formatLegacyEventDateBR(evt.data_evento)}
              </button>
            ))}
          </div>
        )}
            ...
            <div className="flex items-center gap-3 bg-card rounded-xl px-4 py-3 border border-border-color">
              ...
              <div className="text-right">
                ...
                <span className="text-sm font-semibold text-text-primary">
                  {formatDateTimeBR(eventoSelecionado.data_limite)}
                </span>
              </div>
            </div>
          </div>
