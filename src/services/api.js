...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    // Redireciona em 401, exceto nas páginas de login e reset de senha
    if (
...
export const authService = {
  ...
  getPendentes: () => api.get('/auth/pendentes'),
  aprovarUsuario: (id) => api.put(`/auth/aprovar/${id}`),
  rejeitarUsuario: (id) => api.put(`/auth/rejeitar/${id}`),
};
...
export const pedidosService = {
  ...
  getMeusFavoritos: () => api.get('/pedidos/meus-favoritos'),
  getMinhasEstatisticas: () => api.get('/pedidos/minhas-estatisticas'),
  getMinhasConquistas: () => api.get('/pedidos/minhas-conquistas'),
};
...
export const rankingService = {
  getRanking: () => api.get('/sabores/ranking'),
};

// Feedbacks
export const feedbacksService = {
  enviar: (data) => api.post('/feedbacks/', data),
  listar: () => api.get('/feedbacks/'),
};
