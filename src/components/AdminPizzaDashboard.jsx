...
const AdminPizzaDashboard = ({ pedidos, sabores, eventoId }) => {
    ...
    const [numberOverrides, setNumberOverrides] = useState({}); // { pizzaId: assignedNumber } - number swaps
    const [swapModalPizza, setSwapModalPizza] = useState(null); // Pizza being edited for swap
    const [swapNumberPizza, setSwapNumberPizza] = useState(null); // Pizza selected for number swap
        ...
        const loadConfig = async () => {
            try {
                ...
                if (response.data) {
                    ...
                    setNumberOverrides(response.data.number_overrides || {});
                }
            } catch (err) {
        ...
        const saveConfig = async () => {
            ...
            try {
                await pizzaConfigService.save(eventoId, {
                    ...
                    number_overrides: numberOverrides
                });
            } catch (err) {
    ...
    }, [overrides, pairingOverrides, numberOverrides, eventoId, configLoaded]);
    ...
    const pizzas = useMemo(() => {
        ...
        // Sort pedidos by date, then by id for deterministic processing
        // MUST match backend: ORDER BY p.data_pedido, p.id, ip.id
        const sortedPedidos = [...pedidos].sort((a, b) => {
            const dateDiff = parseAppDate(a.data_pedido) - parseAppDate(b.data_pedido);
            if (dateDiff !== 0) return dateDiff;
            return a.id - b.id;
        });

        sortedPedidos.forEach(pedido => {
            // Sort items by id to match backend's ORDER BY ip.id
            const sortedItens = [...pedido.itens].sort((a, b) => a.id - b.id);
        ...
        Object.entries(pairingOverrides).forEach(([halfId1, halfId2]) => {
            ...
            if (h1 && h2 && !pairedSet.has(halfId1) && !pairedSet.has(halfId2)) {
                ...
                const lastUpdate = parseAppDate(h1.lastUpdate) > parseAppDate(h2.lastUpdate) ? h1.lastUpdate : h2.lastUpdate;
        ...
        // 5. Assign Numbers
        const stiPizzas = finalPizzas.filter(p => p.winner === 'STI').sort((a, b) => parseAppDate(b.lastUpdate) - parseAppDate(a.lastUpdate));
        const sgsPizzas = finalPizzas.filter(p => p.winner === 'SGS').sort((a, b) => parseAppDate(a.lastUpdate) - parseAppDate(b.lastUpdate));
    ...
    }, [pedidos, overrides, pairingOverrides, numberOverrides, flavorTypeMap]);
    ...
    // Handle number swap between two pizzas
    const handleNumberSwap = (pizza) => {
        if (!pizza.number) return;

        if (!swapNumberPizza) {
            // First click: select this pizza
            setSwapNumberPizza(pizza);
        } else if (swapNumberPizza.id === pizza.id) {
            // Clicked same pizza: cancel
            setSwapNumberPizza(null);
        } else {
            // Second click: perform swap
            const pizzaA = swapNumberPizza;
            const pizzaB = pizza;
            const numA = pizzaA.number;
            const numB = pizzaB.number;
    ...
    // Don't render until config is loaded to prevent TIE flash
    if (!configLoaded) {
        return (
            <div className="flex items-center justify-center py-12 gap-3 text-text-secondary">
                ...
                Carregando configurações de pizza...
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 animate-fadeIn">
            {/* Number Swap Mode Banner */}
            {swapNumberPizza && (
                <div className="bg-amber-500/20 border border-amber-500/50 rounded-xl p-4 flex items-center justify-between animate-fadeIn">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center text-white font-bold text-lg animate-pulse">
                            {swapNumberPizza.number}
                        </div>
                        <div>
                            <p className="text-amber-300 font-bold">Modo Troca de Número</p>
                            <p className="text-amber-200/70 text-sm">
                                Pizza <strong>#{swapNumberPizza.number}</strong> ({swapNumberPizza.flavorName}) selecionada — clique no número de outra pizza para trocar
                            </p>
                        </div>
                    </div>
                    <button
                        onClick={() => setSwapNumberPizza(null)}
                        className="px-4 py-2 bg-amber-500/30 hover:bg-amber-500/50 text-amber-200 rounded-lg transition-colors text-sm font-medium"
                    >
                        Cancelar
                    </button>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        ...
                        {pizzas.stiPizzas.filter(p => p.isComplete).map(pizza => (
                            <PizzaCard
                                ...
                                onSwap={handleSwap}
                                onSwapHalf={handleOpenSwapModal}
                                onNumberSwap={handleNumberSwap}
                                swapNumberPizza={swapNumberPizza}
                                availableHalves={availableHalves}
                            />
                        ))}
...
const PizzaCard = ({ pizza, onSwap, onSwapHalf, onTieBreak, onNumberSwap, swapNumberPizza, isTie }) => {
    const isSti = pizza.winner === 'STI';
    const isSwapSelected = swapNumberPizza?.id === pizza.id;
    const isSwapTarget = swapNumberPizza && swapNumberPizza.id !== pizza.id && pizza.number;
    const borderColor = isSwapSelected ? 'border-amber-500' : isTie ? 'border-yellow-500' : isSti ? 'border-blue-500' : 'border-green-500';
    const bgColor = isSwapSelected ? 'bg-amber-500/10' : isTie ? 'bg-yellow-500/5' : isSti ? 'bg-blue-500/5' : 'bg-green-500/5';
    ...
    if (pizza.isMeioAMeio) {
        if (pizza.flavorType1 === 'DOCE') color1 = "#5D4037";
        if (pizza.flavorType2 === 'DOCE') color2 = "#5D4037";
        if (pizza.flavorType1 === 'DOCE' && pizza.flavorType2 === 'DOCE') {
            color1 = "#D946EF";
            color2 = "#5D4037";
        }
    } else {
        if (pizza.flavorType === 'DOCE') color1 = "#5D4037";
    }

    return (
        <div className={`card relative overflow-hidden border-2 ${borderColor} ${bgColor} transition-all duration-300 hover:scale-[1.02] ${isSwapTarget ? 'ring-2 ring-amber-500/30' : ''}`}>
            {/* Number Badge - Clickable for swap */}
            {!isTie && pizza.number && (
                <button
                    onClick={() => onNumberSwap?.(pizza)}
                    className={`absolute top-0 left-0 w-12 h-12 flex items-center justify-center text-xl font-bold text-white rounded-br-2xl z-10 transition-all cursor-pointer ${isSwapSelected
                        ? 'bg-amber-500 animate-pulse ring-2 ring-amber-300 scale-110'
                        : isSwapTarget
                            ? 'hover:scale-110 hover:ring-2 hover:ring-amber-400 ' + (isSti ? 'bg-blue-500 hover:bg-amber-500' : 'bg-green-500 hover:bg-amber-500')
                            : (isSti ? 'bg-blue-500 hover:bg-blue-400' : 'bg-green-500 hover:bg-green-400')
                        }`}
                    title={isSwapSelected ? 'Clique em outra pizza para trocar' : isSwapTarget ? `Trocar com #${swapNumberPizza.number}` : 'Clique para trocar número'}
                >
                    {pizza.number}
                </button>
            )}
